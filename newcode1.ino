...
{mc.motorForward(40,40);
delay(300);// right er jonno eita barabi
path[node]='L';
node++;
shortPath();
...
turnleft();}
  ...
  mc.motorForward(-40,-40);
  delay(170);//right er jono delay barai dibi 20ms
  mc.brake();
  reading();
...
int le=lefterror();
int re=righterror();
int MR=MBR+0.75*(le-re);
int ML=MBL+0.75*(re-le);
...
{c++;
delay(1);
if (c==100) break;//jodi turn khub kasakasi hy er detect na kore taile eita komabi 50
straight();
...
delay(500);//block theke ektu jodi agano dorkar hy(jodi turn gula kasakasi thake ei delay barai dibi 100
  mc.motorForward(-MBL,MBR+100);
delay(200);
mc.brake();
mc.motorForward(-MBL,-MBR);
delay(300);//if pishaite hobe beshi mne hy tokhn delay 100 kore barabi er viseversa
mc.motorForward(0,MBR);
delay(3400);
